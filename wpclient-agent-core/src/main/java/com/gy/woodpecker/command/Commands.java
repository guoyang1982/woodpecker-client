...
import com.gy.woodpecker.Exception.CommandInitializationException;
import com.gy.woodpecker.Exception.CommandNotFoundException;
...
import com.gy.woodpecker.tools.GaClassUtils;
import com.gy.woodpecker.tools.GaStringUtils;
import com.gy.woodpecker.tools.WReflectUtils;
...
public class Commands {
    ...
    private Commands() {
        ...
        for (final Class<?> clazz : GaClassUtils.scanPackage(Commands.class.getClassLoader(), "com.gy.woodpecker.command")) {
            ...
            for (final Field field : clazz.getDeclaredFields()) {
                ...
                if (field.isAnnotationPresent(NamedArg.class)) {
                    ...
                    if (arg.hasValue()) {
                        if (opt.has(arg.name())) {
                            ...
                            try {
                                WReflectUtils.setValue(field, value, command);
                    ...
                    // 设置boolean类型,一般只有boolean类型hasValue才为false
                        ...
                        try {
                            WReflectUtils.setValue(field, opt.has(arg.name()), command);
                        } catch (IllegalArgumentException e) {
                            throw new CommandInitializationException(cmdName, e);
                        } catch (IllegalAccessException e) {
                            throw new CommandInitializationException(cmdName, e);
                ...
                // 处理顺序参数
                    ...
                    if (opt.nonOptionArguments().size() > index) {
                        try {
                            WReflectUtils.setValue(field, opt.nonOptionArguments().get(index), command);
                        } catch (IllegalArgumentException e) {
                            throw new CommandInitializationException(cmdName, e);
                        } catch (IllegalAccessException e) {
                            throw new CommandInitializationException(cmdName, e);
